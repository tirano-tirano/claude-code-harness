...
このスキルは「**各コミット・ブランチ作成時のルール**」を扱う。**ブランチ完了後の統合判断**（マージするか PR を出すか等）は `finishing-a-development-branch` スキルが担当する。

```
ブランチでの開発
  ├── git-conventions: 各コミット・ブランチ作成時のルール（このスキル）
  └── finishing-a-development-branch: 開発完了後の統合判断
```

事故が起きた場合は `recovering-from-mistake` スキルを参照（マージ事故、コミット取り消し、ブランチ復旧等）。
