...
`agents/` ディレクトリには、メイン Claude から呼び出される特化エージェントを定義している。
スキル（HOW を決める）と異なり、エージェントは**特定のタスクに特化した独立した Claude セッション**で、
コンテキスト分離して並列実行できる。

| エージェント | 役割 |
|---|---|
| `code-explorer` | 既存コードベースを調査して構造化されたサマリと「必読ファイルリスト」を返す。新機能設計時の類似機能調査、project-migration での既存コードからの feature ファイル生成、仕様変更時の影響範囲特定に使う |
| `code-architect` | 既存パターンを尊重した実装ブループリントを返す。focus（minimal/clean/pragmatic 等）に沿った設計を、複数体並列起動で多面的に比較できる |
| `code-reviewer` | コードレビューを行い、信頼度 80 以上の指摘のみを Critical / Important / NFR 違反 の3グループで返す。CLAUDE.md と feature ファイル、関連 NFR を参照する |

エージェントは並列起動できる（`dispatching-parallel-agents` スキル参照）。
並列起動時は、各エージェントに「必読ファイルリスト」を返させ、メイン Claude が
そのファイルを Read してから次のステップに進む（必須ルール）。
...
| `requesting-code-review` | コードレビュー依頼（信頼度 80 以上の指摘を取得） |
| `receiving-code-review` | レビューフィードバックへの対応 |
| `security-review` | OWASP Top 10:2025 セキュリティチェック |
| `retrospective` | feature 完了時の KPT 形式の振り返り |
...
| `remote-repository` | PR / Issue / CI/CD（feature ファイルから PR description 自動生成パターンあり） |
...
| `claude-md-generator` | CLAUDE.md の生成・更新（フォーク・自作プラグイン用テンプレートあり） |
| `writing-skills` | 新しいスキルの作成方法 |
| `learning-guide` | 開発作業中の概念説明・学習記録 |
